...
var upRight,upLeft,downLeft,downRight;  // Все возможные варианты прогулки для дамкы
...
var multiplier = 1 // 2, если он совершает прыжок 1 в случае контракта
...
function showMoves (piece) {
	/* Если песня была выбрана до того, как мы удалим ее пути, обновленные пути, а не пути Game made by Cojocaru Calin George, все права защищены на новый фрагмент.
избиратель
	...
	*/
	...
	if(!match) {
	 return 0 ; // Если совпадений не найдено; это происходит, например, когда красное движется и вы нажимаете черный
	}

	/*===Теперь в зависимости от их цвета задаю края и движения дамы===*/
	if(the_checker[i].color =="white"){
 	...
 	/*===========ПРОВЕРИТЬ, МОГУ ЛИ Я НАПАСТЬ====*/


		attackMoves(the_checker[i]); // Проверьте, есть ли у меня движение атаки
	

	/*========Если я не могу атаковать, я проверяю, могу ли я пойти======*/
...
function makeMove (index) {
	var isMove = false;
	if(!selectedPiece) // Если игра только началась и ни один трек не был выбран
		return false;
 ...
 /* =========Перспектива движущегося игрока ======*/
	...
	if(mustAttack)  // Чтобы узнать, прыгаю ли я только на один ряд или на 2 
		multiplier = 2;
	else
		multiplier = 1;
		...
		if(index == cpy_upRight){
			...
			if(the_checker[1].color=="white"){
				...
				//Удалить деталь, если прыжок был выполнен
...
/*===========ПЕРЕМЕЩЕНИЕ ЧАСТИ - ИЗМЕНЕНИЕ КООРДИНАТ======*/

function executeMove (X,Y,nSquare){
	// Обменять координаты перемещаемой детали
	the_checker[selectedPieceindex].changeCoord(X,Y); 
	the_checker[selectedPieceindex].setCoord(0,0);
	// Я освобождаю поле, которое занимает фигура, и занимаю то, которое она займет
	block[the_checker[selectedPieceindex].ocupied_square].ocupied = false;			
	//оповещение (the_checker [selectedPieceindex] .ocupied_square);
